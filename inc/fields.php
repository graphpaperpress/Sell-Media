...
function sell_media_add_to_cart_fields( $post_id = null, $attachment_id = null ) {
	...
	<div class="sell-media-add-to-cart-fields">
		...
		<form id="sell-media-cart-items" class="hide">
			<input class="item_number" name="item_number" type="text" value="<?php _e(absint( $post_id ),'sell_media'); ?>" />
			...
			<input class="item_type" name="item_type" type="text" value="<?php  _e( $type,'sell_media' ); ?>" />
			<input class="item_image" name="item_image" type="text" value="<?php _e(sell_media_item_image_src( $post_id, $attachment_id ),'sell_media'); ?>" />
			...
			<input class="item_attachment" name="item_attachment" type="text" value="<?php _e($attachment_id,'sell_media'); ?>" />
		...
		<?php if ( $is_package ) : ?>
			<p class="sell-media-package-excerpt"><?php _e(sell_media_get_excerpt( $post_id ),'sell_media'); ?></p>
			<p class="sell-media-package-excerpt-link sell-media-aligncenter"><a href="<?php _e(get_permalink( $post_id ),'sell_media'); ?>"><?php _e( 'Learn more', 'sell_media' ); ?></a></p>
		<?php endif; ?>
		...
		<div id="sell_media_download_wrapper" class="sell-media-add-to-cart-download-fields">
							...
							<?php
								...
								if ( $prices ) foreach ( $prices as $k => $v ) {
									...
									if ( array_key_exists( $v['id'], $download_sizes['available'] ) || "original" == $v['id'] ) {
										_e('<option value="' . $name . '" data-id="' . $v['id'] . '" data-price="' . number_format( $v['price'], 2, '.', '') . '" data-qty="1" data-size="' . $dimensions . '">' . $name  . ': ' . sell_media_get_currency_symbol() . sprintf( '%0.2f', $v['price'] ) . '</option>','sell_media');
									}
								}
							?>
						</select>
			...
			<?php else : ?>
				<input id="sell_media_item_base_price" type="hidden" value="<?php _e(sanitize_text_field($price),'sell_media'); ?>" data-price="<?php _e($price,'sell_media'); ?>" data-id="original" data-size="original" />
			<?php endif; ?>
		...
		<div class="total-container cf">
			<strong><?php _e( 'Total', 'sell_media' ); ?>:</strong> <span class="price-container"><?php _e(sell_media_get_currency_symbol(),'sell_media'); ?><span id="total" class="item_price" data-price=<?php _e($price,'sell_media'); ?>><?php _e($price,'sell_media'); ?></span></span>
		</div>

		<div class="button-container cf">
			<p id="sell-media-add-to-cart"><?php _e(sell_media_item_add_to_cart_button( $post_id, $attachment_id, null, null, true, $type ),'sell_media'); ?></p>
			<p id="sell-media-add-to-lightbox"><?php _e(sell_media_lightbox_link( $post_id, $attachment_id ),'sell_media'); ?></p>
		</div>
