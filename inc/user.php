...
{
	if(is_user_logged_in()){echo '<p class="p-sell"><a href="'.site_url( 'dashboard' ).'" title="">'.esc_html_e( 'Dashboard', 'sell_media' ).'</a> or <a href="'.wp_logout_url().'" title="">'.esc_html_e( 'Logout', 'sell_media' ).'</a> for faster checkout</p>';
	}else
	{
		echo '<p class="p-sell"><a href="'.wp_login_url( get_permalink() ).'" title="">'.esc_html_e( 'Login', 'sell_media' ).'</a> or <a href="'.wp_registration_url().'" title="">'.esc_html_e( 'create an account', 'sell_media' ).'</a> for faster checkout</p>';
	}
}
